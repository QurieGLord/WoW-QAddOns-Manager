...
class AddonSearchService {
  ...
  Future<List<AddonItem>> searchAll(String query, String gameVersion) async {
    ...
    final results = await Future.wait(
      _providers.map(
        (provider) => provider.search(normalizedQuery, normalizedVersion).catchError((_) => <AddonItem>[]),
      ),
    );
  ...
  Future<({String url, String fileName})?> getDownloadInfo(AddonItem item, String gameVersion) async {
    ...
    try {
      ...
      for (final provider in _providers) {
        if (item.providerName == GitHubProvider.staticProviderName && provider is GitHubProvider) {
          final info = await provider.getDownloadUrl(item, normalizedVersion);
          if (info != null && info.url.isNotEmpty) {
            return info;
          }
        }
        if (item.providerName == CurseForgeProvider.staticProviderName && provider is CurseForgeProvider) {
          final info = await provider.getDownloadUrl(item, normalizedVersion);
          if (info != null && info.url.isNotEmpty) {
            return info;
          }
