...
class CurseForgeClient {
    ...
    try {
      ...
      final attempts = <String?>[
        if (requestedVersion.isNotEmpty &&
            !profile.apiVersionCandidates.contains(requestedVersion.toLowerCase()))
          requestedVersion,
  ...
  int _scoreFile(CfFile file, WowVersionProfile profile) {
    final explicitVersions =
        file.gameVersions.where((version) => version.trim().isNotEmpty).toList(growable: false);
    ...
    final descriptiveScore = profile.numericCompatibilityScore(descriptiveMetadata);
    ...
    if (hasStrictExplicitMatch) {
      ...
      if (explicitVersions.any((version) => _matchesExactVersion(version, profile))) {
        score += 40;
      } else {
        score += 20;
      }
    ...
    if (descriptiveScore > 0) {
      final descriptiveBonus =
          (file.displayName != null && profile.containsRequestedVersion(file.displayName!))
              ? 15
  ...
  bool _matchesRequestedBranch(String value, WowVersionProfile profile) {
    ...
    if (candidate.hasNumericVersion && candidate.majorMinor == profile.majorMinor) {
      return true;
    }
  ...
  bool _matchesExactVersion(String value, WowVersionProfile profile) {
    ...
    final candidate = WowVersionProfile.parse(value);
    return candidate.hasNumericVersion && candidate.exactVersion == profile.exactVersion;
  }
